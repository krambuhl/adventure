...
const [size, offset] = [Math.pow(2, 4), 3]
        ...
        draw={(p, { frame }) => {
          ...
          for (let x = 0; x < stepX; x++) {
            for (let y = 0; y < stepY; y++) {
              ...
              p.sphere(sizeX * 0.5, resolution, resolution)
              p.pop()
            }
          }
        }}
...
const vs = `
  ...
  void main() {
    ...
    float distortion = sin(positionVec4.x * (frequency * amplitude) + sin(uFrameCount * 0.01));
    positionVec4.x += (sin(x / uStepX) + 1.) * (cos(aNormal.x) * sin(x));
    positionVec4.z += tan(uFrameCount * 0.01);
    // positionVec4.z = (
    //   positionVec4.y *
    //   (distortion * (((y / uStepY) + 1.) / 0.8)) * (amplitude * (frequency * x * y))
    // );
