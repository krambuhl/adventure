...
function manipulate(backContext, context, w, h, frame) {
  ...
  for (let i = 0; i < imageData.data.length; i += 4) {
    ...
    const pos = i
    let [r, g, b, a] = [
      0 + Math.floor((Math.sin(frame * 30 * ((x / 1000) * (frame / 1000     ))) + 1) * w) * 4, 
      1 + Math.floor((Math.cos(frame * 30 * ((x / 1000) * (frame / 1000     ))) + 1) * w) * 4, 
      2 + Math.floor((Math.tan(frame * 30 * ((x / 1000) * (frame / 1000     ))) + 1) * w) * 4, 
      3
    ]
    const swipe =  0 ?? (frame * 4 * 10) % (size / 8)
